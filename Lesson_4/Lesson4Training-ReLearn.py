...
class Fetch_ReLearn():
    ...
    def __init__(self, direction1=1, direction2=1):
        ...
        self.motor1 = hub.port.C.motor    #### REPLACE WITH YOUR SPIKE'S HUB PORTS 
        self.motor2 = hub.port.D.motor    #### <----------------------------------
        self.touch = hub.port.F.device    #### <----------------------------------
    ...
    def reinforcement_learning(self):
        ...
        if self.training_count < 2:
            ...
            self.training_count += 1
        while True:
            print('Press CENTER button to let the puppy give it a try')
            print('t=', time)
            while not hub.button.center.is_pressed():
                utime.sleep(0.01)
            ...
            while True:
                ...
                # center button to let it try again
                utime.sleep(0.01)
                if self.touch.get()[0]>0:
    ...
    def prediction_entry(self):
        ...
        while not hub.button.left.is_pressed():
            ...
            # Puppy tries moving
            self.puppy_move_forward(time)
            print('CENTER button: try again, LEFT button: exit')
