...
import hub, utime, force_sensor, color_sensor, color
...
class PickyEater:
    ...
    def __init__(self):
        ...
        self.colorsensor = port.B #<---- Modify this to your dog's ports
        ...
        self.forcesensor = port.E #<---- Modify this to your dog's ports
    ...
    def watch(self):
        ...
        while True:
            ...
            if samplecount % 50 == 0:
                if samplecount == 0:
                    print('*'*20)
                    print('Press LEFT to take data for the current color')
                    print('*'*20)
                else:
                    print('*'*20)
                    print('Press FORCE SENSOR to quit training')
                    print('Press LEFT to take more data for the current color')
                    print('Press RIGHT to train another color')
                    print('*'*20)
                while True:
                    if force_sensor.pressed(self.forcesensor) and samplecount != 0:
                        # Press CENTER button to quit data recording
                        print('Finished data recording for %s Colors' %
                            (self.colorcount + 1))
                        break
                    if button.pressed(button.RIGHT) and samplecount != 0:
                    ...
                    if button.pressed(button.LEFT):
                        ...
                        if samplecount == 0:
                            print('Recording Data for Color #1')
                        else:
            ...
            self.colors = []
            self.colors.append(color_sensor.rgbi(self.colorsensor)[0])# R values
            self.colors.append(color_sensor.rgbi(self.colorsensor)[1])# G values
            self.colors.append(color_sensor.rgbi(self.colorsensor)[2])# B values
    ...
    def prediction(self, k, printresult=False):
        ...
        self.colors = []
        self.colors.append(color_sensor.rgbi(self.colorsensor)[0])
        self.colors.append(color_sensor.rgbi(self.colorsensor)[1])
        self.colors.append(color_sensor.rgbi(self.colorsensor)[2])
